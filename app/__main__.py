...
def detect_bowl_mask(arr: np.ndarray) -> tuple[np.ndarray, str, dict | None]:
    ...
    if model is None:
        return (
            ...
            load_error or "yolo unavailable",
            None,
        )
...
def _estimate_kibble_stats(
    *,
    ...
    val = gray
    brown_mask = (saturation >= 0.18) & (val >= 0.10) & (val <= 0.92)
    dark_kibble_mask = (saturation >= 0.10) & (val >= 0.06) & (val <= 0.58)
    texture_mask = (
